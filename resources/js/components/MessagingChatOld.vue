  ...
  <div>
		...
		<div class="px-16 mt-4">
			<div class="headingoflist">
				...
				<button class="add-light-button" @click='deleteView(emp_id, userable_type)'>
					<font-awesome-icon :icon="['far', 'trash-alt']" class="text-gray-500 font-size-24" />
					Delete Thread</button>
			</div>

		</div>
		<div class="px-16 mt-8 main_message_section" v-if="messages.length != 0">
			<div class="chat_section" v-for="(data, index) in messages" :key="'A'+index">
				<div v-if="index == messages.length -1 " class="subject">Subject: {{data.subject}}</div>
			...
			<div class="messages flex m-5 py-1 px-5"  v-for="data in messages" :key="data.id" :class="data.from=='admin'?'reverse-row':'no-row-reverse'">
				<span class="logo">
					<span class="image" >
							<img v-if="data.user.client_image != undefined || data.user.client_image != null" :src="currentpath+'/storage/'+ data.user.client_image" />
					</span>
				</span>
				<div class="message-detail ml-2 p-2 mr-2" :class="data.from=='admin'?'reverse-color':'no-color-reverse'" >
				<p v-if="data.message_type =='text'" :class="data.from=='admin'?'reverse-text':'no-text-reverse'">{{data.message}}</p>
				...
				<p v-else :class="data.from=='admin'?'reverse-text':'no-text-reverse'">{{data.message}}</p>
				</div>
			...
			<div class="reply-box flex mt-5">
				<div class="left-section">
					<span class="input-box">
						<textarea class="p-5" v-model="reply"  />
					</span>
...
export default {
	...
	data() {
		return {
			...
			userable_type:this.$route.params.type,
			Images:{
				selectedFile : null
			},
			messages:[],
	...
	created() {
		let vm = this;
		vm.getMessages(this.emp_id, this.userable_type);
	...
	methods: { 
		...
		Images_onUpload() {
			...
			mypostparameters.append('userable_id', this.userable_id );
			mypostparameters.append('userable_type', this.userable_type );
			...
			axios.post('/api/adminmessage', mypostparameters).then(res => {
					if(res.data.status){
					vm.$swal({
					icon: "success",
		...
		deleteView(id, type){
			...
			let vm = this;
			axios
				.post(`/api/messagedelete/${id}/${type}`)
				.then(res => {
					if(res.data.status){
					vm.$swal({
					icon: "success",
		...
		// get messages
		getMessages(id, userable_type){
			let vm = this;
			axios
				.get(`/api/message/${id}/${userable_type}`)
				.then(res => {
					if(res.data.status){
						vm.messages = res.data.data
						vm.subject = vm.messages[0].subject
						vm.userable_id = vm.messages[0].userable_id
						vm.userable_type = vm.messages[0].userable_type
					}
					else{
						vm.messages = [];
					}
