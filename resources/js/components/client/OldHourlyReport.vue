...
export default {
  ...
  mounted(){
    ...
    let vm = this;
        axios
          .post(`/api/hourlyfilter`, {search: x, report_type_id: 1,client_id: this.user_id, current_date:  vm.modal.date  }) 
          .then(res => {
    ...
    sendDate(){
        var y = document.getElementById("location").value;
      		let vm = this;
      axios.post(`/api/hourlyfilter`, {search: y, current_date : vm.modal.date , client_id : this.user_id ,report_type_id: 1}) 
          .then(res => {
        if(res.data.status != true){
           vm.$swal({
              icon: 'error',
