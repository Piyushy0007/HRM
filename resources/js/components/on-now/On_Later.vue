  ...
  <div>
    ...
    <div align="center">
      ...
      <table class="modwide">
        <tbody>
          <tr>
            <td class="wgt"><b class="titlebox">Scheduled On Later</b></td>
          </tr>
          <tr>
            <td class="bWGT">
              <table
                ...
                <thead>
                 ...
                 <tbody v-if="index.employees.length===0">
                  ...
                  <tr v-for="(data, index) in index.employees" :key="data.id">
                    ...
                    <td class="text-center">{{data.paid_hours}}</td>
...
export default {
  ...
  methods: {
    ...
    indexEmployee() {
      let vm = this;
      axios.get(`/api/employees/on_later`).then((res) => {
        if(res.data.status){
        vm.index.employees = res.data.data;
        vm.index.employees = vm.index.employees.filter(x =>{
          return x.start = moment(x.added_to + ' ' + x.start).format('HH:mm A')
        })
        vm.index.employees = vm.index.employees.filter(x =>{
          return x.end = moment(x.added_to + ' ' + x.end).format('HH:mm A')
        })
        }
        else{
          vm.$swal({
