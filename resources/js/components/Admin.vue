<template>
  <div class="c-employee-index client-table">
    ...
    <modal v-model="modal.createclient" class="modal-add-new-employee" size="md:w-5/12" title="Create Admin">
        ...
        <form @submit.prevent="handleSubmit(storeEmployee('create'))" ref="frmcreateclient" novalidate>
            ...
            <ValidationProvider rules="required" v-slot="v">
      				<div class=" md:items-center">
      					<div class="md:w-4/4">
      						<label class="block mb-1 md:mb-0 pr-4">First Name<span class="req_form_fields">*</span></label>
      					</div>
      					<div class="md:w-4/4">
            ...
            <ValidationProvider rules="required" v-slot="v">
      				<div class="md:items-center">
      					<div class="md:w-4/4">
      						<label class="block mb-1 md:mb-0 pr-4">Last Name<span class="req_form_fields">*</span></label>
      					</div>
      					<div class="md:w-4/4">
      						<input id="textbox" class="appearance-none block w-full rounded py-1 px-4 leading-tight focus:outline-none border-2 border-gray-200" type="text" v-model="modal.createClient.phone">
      					</div>
      				</div>
              <div class="md:flex md:items-center mb-1">
  ...
  async deleteView(data){
    let vm = this;
      try {
          if (confirm(`Are you sure you want to delete this admin?  `)) {
           await axios.get(`/api/deleteadmin/${data.id}`, { headers: this.headers}).then(res => {
                vm.$swal({
   ...
   async resetPassword(data){
    ...
    storeEmployee(value) {
      ...
      if ( vm.modal.createClient.clientname == '' || vm.modal.createClient.phone == ''  || vm.modal.createClient.email == '' ) {
        vm.isLoader = false
        vm.$swal({
          icon: 'error',
        ...
        axios
        .post('/api/addadmin', Object.assign(vm.modal.createClient))
        .then(res => {
         if(res.data.status == true){
          vm.modal.createClient = {}
          vm.$swal({
